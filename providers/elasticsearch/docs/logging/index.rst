...
On Airflow 3, the built-in write modes (``write_to_es`` and ``write_stdout``) stream each log record to Elasticsearch or stdout in near-real-time while the task is running, so a running task's logs can be read from Elasticsearch just like CloudWatch remote logging. This streaming requires the default ``[logging] log_filename_template``; with a customized template the logs are shipped in a single batch when the task finishes instead.
