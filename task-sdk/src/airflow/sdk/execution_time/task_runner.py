...
def _resolve_runtime_entrypoint(startup_details: StartupDetails, log: Logger) -> Callable[[], None] | None:
    ...
    1. If the task's ``language`` field is set, match it directly against
    ...
    # Fallback: resolve runtime name from queue mapping when language is not set.
    if language is None:
        from airflow.sdk.execution_time.coordinator import QueueToRuntimeCoordinatorMapper

        log.debug(
            "No language specified for task, attempting to resolve runtime from queue mapping",
            queue=startup_details.ti.queue,
            task_id=startup_details.ti.task_id,
        )
        language = QueueToRuntimeCoordinatorMapper.from_config().resolve(startup_details.ti.queue)
        if language is None:
            log.debug(
