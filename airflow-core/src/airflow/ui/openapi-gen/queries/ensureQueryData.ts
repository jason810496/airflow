...
* @throws ApiError
*/
export const ensureUseTaskLogStreamServiceStreamTaskLogData = (queryClient: QueryClient, { dagId, dagRunId, mapIndex, resumeToken, taskId, tryNumber }: {
  dagId: string;
  dagRunId: string;
  mapIndex?: number;
  resumeToken?: string;
  taskId: string;
  tryNumber: number;
}) => queryClient.ensureQueryData({ queryKey: Common.UseTaskLogStreamServiceStreamTaskLogKeyFn({ dagId, dagRunId, mapIndex, resumeToken, taskId, tryNumber }), queryFn: () => TaskLogStreamService.streamTaskLog({ dagId, dagRunId, mapIndex, resumeToken, taskId, tryNumber }) });
/**
* List Teams
* @param data The data for the request.
* @param data.limit
