# Execution API — Development Guide

The Execution API (`airflow-core/src/airflow/api_fastapi/execution_api/`) handles communication
between task workers and the API server. Workers and API servers deploy independently, so
backward compatibility is critical — older clients must work with newer servers.

## Versioning with Cadwyn

This API uses [Cadwyn](https://github.com/zmievsa/cadwyn) with CalVer (`vYYYY_MM_DD.py`).

### When Making Changes

1. Check the latest version file in `versions/`. If its date is in the future (unreleased),
   add your `VersionChange` class to that file. Otherwise create a new `vYYYY_MM_DD.py`.
...
- Response changes: implement `@convert_response_to_previous_version_for(Model)` and check
  field existence before popping.
...
Adding a new Execution API feature touches multiple packages that must stay in sync:
...
7. **DAG processor & triggerer exclusions** — these use `InProcessExecutionAPI` and have explicit
   message type unions. Add new types to their handler or exclusion lists in
   `airflow/dag_processing/processor.py` and `airflow/jobs/triggerer_job_runner.py`.
8. **Regenerate models** — `cd task-sdk && python dev/generate_task_sdk_models.py`.
9. **Tests** — add tests in all of these:
   ...
   - `task-sdk/tests/task_sdk/execution_time/test_supervisor.py` — add a `RequestTestCase` entry to `REQUEST_TEST_CASES`
...
- Full versioning guide: `contributing-docs/19_execution_api_versioning.rst`

## Token Scope Infrastructure

Token types (`"execution"`, `"workload"`), route-level enforcement via `ExecutionAPIRoute` +
`require_auth`, and the `ti:self` path-parameter validation are documented in the module
docstring of `security.py`.
