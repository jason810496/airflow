...
``<sdk>/generated/lang-sdk/capabilities.json`` file which is the single source consumed here. This module owns
the *schema* of that file, the *registry* of SDKs, and the *renderers* for the two matrices:

* the per-SDK Markdown table embedded in each ``<sdk>/README.md`` (rendered by that SDK's own
  prek hook via :func:`render_markdown_table`), and
* the consolidated cross-SDK reStructuredText ``list-table`` in the language-SDK docs
  (rendered by ``update_lang_sdk_central_matrix.py`` via :func:`render_central_table`).
...
CENTRAL_MATRIX_HEADER = ".. BEGIN AUTO-GENERATED LANG-SDK COMPAT MATRIX"
CENTRAL_MATRIX_FOOTER = ".. END AUTO-GENERATED LANG-SDK COMPAT MATRIX"

SUPPORTED_MARK = "✓"
UNSUPPORTED_MARK = "✗"
NA_MARK = "n/a"  # a gated native-Dag capability while native-dag-authoring is unsupported
ABSENT_MARK = "–"  # SDK has not published a capabilities.json yet
...
# whole Dag in the target language and are gated by NATIVE_DAG_GATE (except the gate itself and
# object-store, which is usable independently of native-Dag authoring).
CAPABILITY_DIMENSIONS: list[Capability] = [
    ...
    Capability("object-store", "MAY", "native", False),
]
...
LEGEND = (
    ...
    f"{NA_MARK} not applicable · {ABSENT_MARK} not published. A tier marked † applies only "
    f"when `{NATIVE_DAG_GATE}` is supported."
)
...
# Each SDK's dumper writes its manifest to generated/lang-sdk/capabilities.json under the SDK root
# (a generated artifact kept out of the hand-authored source tree).
...
def _since(entry: DimensionEntry) -> str:
    if not entry.get("supported"):
        return ABSENT_MARK
    return entry.get("since") or ABSENT_MARK
...
def _state_cell(doc: CapabilitiesDoc | None, name: str) -> str:
    if doc is None:
        return ABSENT_MARK
    return _state_mark(doc["states"][name])


def _capability_cell(doc: CapabilitiesDoc | None, cap: Capability) -> str:
    if doc is None:
        return ABSENT_MARK
    return _capability_mark(doc, cap)


def render_central_table(docs_by_id: dict[str, CapabilitiesDoc | None]) -> list[str]:
    """Render the consolidated cross-SDK RST ``list-table`` as a list of lines.

    ``docs_by_id`` maps every registered SDK id to its loaded document, or ``None`` when that SDK
    has not published a ``capabilities.json`` yet (its column then shows the "absent" mark).
    """
    ordered = [(sdk["id"], sdk["display"], docs_by_id.get(sdk["id"])) for sdk in LANG_SDKS]

    def row(label: str, values: list[str]) -> list[str]:
        # rstrip each cell so an empty cell renders as "     -" without trailing whitespace
        # (otherwise the trailing-whitespace hook and this generator would fight over the file).
        return [f"   * - {label}\n", *[f"     - {value}".rstrip() + "\n" for value in values]]

    def header_row(label: str) -> list[str]:
        return row(f"**{label}**", ["" for _ in ordered])

    widths = " ".join(["30"] + ["15"] * len(ordered))
    lines = [
        "\n",
    ...
    lines += row("Dimension", [display for _, display, _ in ordered])
    lines += row(
        "Min. Airflow version", [doc["min_airflow_version"] if doc else ABSENT_MARK for _, _, doc in ordered]
    )
    lines += row(
        "Supervisor schema",
        [doc["supervisor_schema_version"] if doc else ABSENT_MARK for _, _, doc in ordered],
    )
    lines += header_row(STATES_GROUP_LABEL)
    for state, tier in STATE_DIMENSIONS:
        lines += row(f"``{state}`` ({tier})", [_state_cell(doc, state) for _, _, doc in ordered])
    current_group = ""
    for cap in CAPABILITY_DIMENSIONS:
        if cap.group != current_group:
            current_group = cap.group
            lines += header_row(GROUP_LABELS[cap.group])
        lines += row(
            f"``{cap.name}`` ({_tier_label(cap)})", [_capability_cell(doc, cap) for _, _, doc in ordered]
        )
    lines.append("\n")
    lines.append(
        f"*Marks:* ``{SUPPORTED_MARK}`` supported · ``{UNSUPPORTED_MARK}`` not supported · "
        f"``{NA_MARK}`` not applicable · ``{ABSENT_MARK}`` not published. A tier marked ``†`` "
        f"applies only when ``{NATIVE_DAG_GATE}`` is supported.\n"
    )
    lines.append("\n")
    return lines
