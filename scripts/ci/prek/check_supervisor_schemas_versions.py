...
``task-sdk/src/airflow/sdk/execution_time/supervisor_schemas/versions/``.
...
def _upstream_has_supervisor_schemas() -> bool:
    """Return True if the target branch carries the supervisor_schemas package."""
...
def main() -> int:
    ...
    # Files under supervisor_schemas/ that reference the bundle's
    ...
    if not _upstream_has_supervisor_schemas():
        ...
        console.print(
            ...
            "has no supervisor_schemas package yet. The check activates once "
            "this PR merges."
        )
        return 0
